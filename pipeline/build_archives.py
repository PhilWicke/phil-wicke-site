...
def build_imprint():
    ...
    body = (
        ...
        '<p>Philipp Wicke</p>'
        # the postal address is an image of glyph paths (generated by gen_address_svg.py from the
        # gitignored address.local), assembled by JS with a noscript fallback — visible to every
        # visitor immediately (per § 5 DDG), but the address never appears as text in HTML or repo
        '<p id="addr" style="margin-top:6px;"><noscript><img src="assets/e5b1.svg" alt="Anschrift"></noscript></p>'
        "<script>document.getElementById('addr').insertAdjacentHTML('afterbegin','<img src=\"assets/'+['e5','b1'].join('')+'.svg\" alt=\"Anschrift\">');</script></section>"
